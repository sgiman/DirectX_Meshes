...
//		К счастью для нас, при использовании мешей D3DX делает большую часть работы.
//		Это разбор файла геометрии и создание буферов вершин
//		(и индексные буферы). В этом руководстве показано, 
//		как использовать D3DXMES объект, включая его загрузку из файла и отрисовку. 
//		Только один D3DX не обрабатывает материалы и текстуры для сетки,
...
//		независимо от того, как сетка создана, мы можем добавлять/удалять нормали и
//		также добавлять многие текстурные массивы координат (для мультитекстурирования) и т.д.
//
